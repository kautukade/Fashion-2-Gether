...
export default function AdminHomepage() {
  ...
  useEffect(() => {
    if (!isSupabaseConfigured() || !supabase) {
      setSections(demoStore.getHomepage());
      setLoading(false);
      return;
    }
  ...
  const saveLocal = (next: any[]) => {
    setSections(next);
    demoStore.setHomepage(next);
  };

  const toggleVisibility = async (id: string, current: boolean) => {
    const next = sections.map(s => s.id === id ? { ...s, is_visible: !current } : s);
    if (!isSupabaseConfigured() || !supabase) {
      saveLocal(next);
      return;
    }
  ...
  return (
    <div>
      <h1 className="text-2xl font-display text-charcoal mb-2">Homepage CMS</h1>
      <p className="text-sm text-gray-500 mb-2">Control which sections appear on the homepage and their labels.</p>
      {!isSupabaseConfigured() && <p className="text-xs text-amber-600 mb-6">Demo mode: CMS changes are saved in this browser.</p>}
          ...
          {sections.map((section, i) => (
            <div key={section.id} className="flex items-center gap-4 px-4 py-3 hover:bg-gray-50/50">
              ...
              <div className="flex-1">
                <input type="text" value={section.title || section.section_key} onChange={(e) => updateTitle(section.id, e.target.value)}
              ...
              <button onClick={() => toggleVisibility(section.id, section.is_visible)}
